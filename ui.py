"""PySide6 user interface for the macOS companion app."""
...
from bridge_server import LocalChatBridge
...
class MainWindow(QMainWindow):
    def __init__(self) -> None:
        ...
        self.bridge = LocalChatBridge(
            api_key_provider=lambda: self.api_key,
            model_provider=lambda: self.model_input.text().strip() or DEFAULT_MODEL,
        )
    ...
    def build_settings_panel(self, save_button: QPushButton, clear_button: QPushButton) -> QWidget:
        ...
        bridge_label = QLabel(f"Extension bridge\n{self.bridge.url}")
        bridge_label.setObjectName("bridgeLabel")
    ...
    def start_bridge(self) -> None:
        try:
            ...
            self.append_system_message(f"Extension bridge could not start: {exc}")
